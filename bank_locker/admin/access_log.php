...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt->execute()) $msg = "Access logged successfully!";
    else $err = "Error: " . $conn->error;
}

// Get active allocations for form
    ...
    <form method="POST">
      <div class="form-grid">
        ...
        <div class="form-group">
          ...
          <input type="text" name="approved_by" class="form-control" required placeholder="Staff name / ID">
        </div>
      </div>
      <div class="form-group">
        <label class="form-label">Purpose of Visit</label>
        <input type="text" name="purpose" class="form-control" placeholder="e.g. Deposit jewelry, Retrieve documents">
      </div>
      <button type="submit" class="btn btn-primary">📝 Log Access</button>
    </form>
  </div>
</div>
