...
$sql = "SELECT a.*, c.full_name, c.customer_id AS cid, l.locker_number, l.locker_size
        FROM allocations a JOIN customers c ON a.customer_id=c.id JOIN lockers l ON a.locker_id=l.id WHERE 1=1";
$params = []; $types = '';
...
if ($status_filter) { $sql .= " AND a.status = ?"; $params[] = $status_filter; $types .= 's'; }
if ($date_from) { $sql .= " AND a.allocation_date >= ?"; $params[] = $date_from; $types .= 's'; }
if ($date_to) { $sql .= " AND a.allocation_date <= ?"; $params[] = $date_to; $types .= 's'; }
    ...
    <form method="GET">
      <div class="form-grid">
        <div class="form-group">
          ...
          <input type="text" name="q" class="form-control" placeholder="Name / ID / Locker / Alloc No." value="<?= htmlspecialchars($search) ?>">
        </div>
        <div class="form-group">
...
<div class="card">
  <div class="card-header"><h3>📋 Results (<?= $allocations->num_rows ?>)</h3></div>
  <div class="table-responsive">
    <table>
      <thead><tr><th>#</th><th>Alloc No.</th><th>Customer</th><th>Locker</th><th>Size</th><th>Date</th><th>Expiry</th><th>Rent</th><th>Payment</th><th>Status</th></tr></thead>
      <tbody>
        ...
        <tr><td colspan="10" class="text-center" style="padding:30px;color:#888;">No results found.</td></tr>
        <?php endif; ?>
      </tbody>
    </table>
  </div>
